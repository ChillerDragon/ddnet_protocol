...
// Used by the flag carrier fields in the `DDNetObjGameData` snap item.
...
typedef struct
{
	...
	// The size is also sent over the network.
	// The is the amount of packed integers in the `data` field.
	// Warning! This is not the amount of bytes in `data`!
	int32_t size;
	...
	// It is the size of the `data` in bytes.
	size_t data_len;
} DDNetObjUnknown;
...
typedef struct
{
	...
	int32_t x;
	int32_t y;
	// Be careful to not confuse this with the generic snap item field `type_id`
	// The `type` is pickup specific and determines if it is a weapon, ninja, health or armor.
	DDNetPowerup type;
	// The `subtype` should only be used if the `type` is set to `DDNET_POWERUP_WEAPON`
	// otherwise `subtype` should be set to `0` by the sender and ignored by the receiver.
...
// Info about the current game state and score.
// This object should only exist once per snapshot.
typedef struct
{
	int32_t type_id;
	int32_t id;
...
// This snap item is only expected to be included in the snapshot
// if the `DDNetObjGameInfo` `game_flags` are set.
//
...
typedef struct
{
	...
	// Needs the `DDNetObjGameInfo.game_flags` set to `DDNET_GAMEFLAG_TEAMS`
	// to work properly.
	...
	// to work properly.
	// TODO: which type to use for that field? Could use `DDNetFlag` but that does not clearly communicate it also contains client ids
	int32_t flag_carrier_red;

	// Client Id of the current flag holder. Is expected to be `-1` if the blue flag
	// is not being carried by anyone. See the `DDNetFlag` enum for all special values.
	// Needs the `DDNetObjGameInfo.game_flags` set to `DDNET_GAMEFLAG_FLAGS`
	// to work properly.
...
// This is never sent over the network on its own.
// It is always used as a field in the `DDNetObjCharacter` object.
typedef struct
{
	int32_t type_id;
	int32_t id;
...
// Represents a tee in the world.
// Its position and other state.
typedef struct
{
	int32_t type_id;
	int32_t id;
...
typedef struct
{
	...
	// This is set to true if this player is the same player as the snapshot receiver.
	// So the client can use this snap item to figure out its own client id.
	bool local;
	int32_t client_id;
	DDNetTeam team;

	// The amount of points a player made, will be displayed as is in the scoreboard.
	// Unless it is a ddrace server that uses time score then a time should be displayed.
	// This score used to also hold times as negative numbers so shorter times would
	// be sorted higher by vanilla clients. But nowerdays modern ddnet also sends
	// the timescore in dedicated snapshot item extensions. These are not supported
	// yet by the ddnet_protocol library.
	int32_t score;
